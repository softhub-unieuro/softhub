...
import { X, ArrowLeft, ChevronDown, ShieldAlert, Check } from 'lucide-react';
...
export function BarraPrevisualizacaoCargo() {
    ...
    return (
        <div className="bg-[#4e5dec] text-white px-4 py-2 flex items-center justify-between sticky top-0 z-[100] shadow-xl border-b border-indigo-400/30 animate-in slide-in-from-top duration-300">
            {/* Esquerda: Voltar às configurações */}
            <button
                onClick={() => navigate('/admin/configuracoes')}
                className="flex items-center gap-2 px-3 py-1.5 rounded-md hover:bg-white/10 transition-colors border border-white/20 text-[11px] font-bold"
            >
                <ArrowLeft size={14} />
                Voltar às configurações de cargo
            </button>

            {/* Centro: Status e Botão Desativar */}
            <div className="flex items-center gap-4">
                <span className="text-[12px] font-medium text-indigo-50">
                    Você está vendo este servidor como <strong className="text-white">{roleVisualizacao}</strong>.
                </span>
                
                <button
                    onClick={() => setRoleVisualizacao(null)}
                    className="px-4 py-1 rounded-md border border-white/40 hover:bg-white/10 text-[11px] font-black uppercase tracking-tight transition-all active:scale-95"
                >
                    Desativar
                </button>
            </div>
            ...
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <button className="flex items-center gap-2 px-3 py-1.5 rounded-md hover:bg-white/10 transition-colors border border-white/20 text-[11px] font-bold min-w-[140px] justify-between">
                        Selecionar cargos
                        <ChevronDown size={14} className="opacity-50" />
                    </button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-[200px] mt-1 bg-[#2f3136] border-none text-gray-300 rounded-lg p-2 shadow-2xl">
